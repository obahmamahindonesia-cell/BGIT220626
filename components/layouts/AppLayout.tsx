...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      <div className={`flex items-center justify-between px-5 h-16 border-b border-white/10 ${collapsed ? 'justify-center px-0' : ''}`}>
      ...
      <div className={`px-5 py-4 border-t border-white/10 ${collapsed ? 'flex flex-col items-center px-2' : ''}`}>
        {collapsed ? (
          <>
      ...
      <div className={`px-5 py-3 border-t border-white/10 ${collapsed ? 'flex justify-center px-2' : ''}`}>
        {collapsed ? (
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-400 truncate max-w-[160px]">{userName || 'Pengguna'}</p>
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F9FC]">
      <aside className={`hidden lg:flex flex-col fixed left-0 top-0 z-30 h-screen bg-gradient-to-b from-[#0B1F3A] to-[#123E7C] transition-all duration-300 ease-in-out ${

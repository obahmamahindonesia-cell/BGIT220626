...
import { Maximize2, Minimize2, Flag, AlertTriangle } from 'lucide-react'
      ...
      <div className="h-full max-w-screen-2xl mx-auto px-4 lg:px-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden md:flex items-center gap-2">
            <span className="text-xs text-white/40">Tes</span>
            {currentQuestion && (
              <>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-1.5 ml-1">
              ...
              <span className={`text-[10px] font-medium ${isCritical ? 'text-red-400' : isWarning ? 'text-amber-400' : 'text-white/30'}`}>
                {isCritical ? 'Hampir habis' : isWarning ? `${Math.ceil(timeRemaining / 60)} menit lagi` : 'tersisa'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => {
                if (confirm('Apakah Anda yakin ingin mengakhiri sesi tes ini?')) {
                  finishTest()
                }
              }}

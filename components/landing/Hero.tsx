...
const LEVELS = [
  { code: 'A1', color: '#6B7280' },
  { code: 'A2', color: '#6B7280' },
  { code: 'B1', color: '#378ADD' },
  { code: 'B2', color: '#378ADD' },
  { code: 'C1', color: '#C8102E' },
  { code: 'C2', color: '#C9A84C' },
]
...
export default function Hero() {
  ...
  return (
    <section className="bg-[#0B1F3A] text-[#F8F6F1] pt-28 pb-16 px-6 text-center">
      <div className="max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 bg-[#C9A84C]/15 border border-[#C9A84C]/40 text-[#C9A84C] text-xs font-medium tracking-wider px-4 py-1.5 rounded-full uppercase mb-6">
          Inovasi Penilaian Bahasa Indonesia
        </div>

        <h1 className="font-[family-name:var(--font-playfair)] text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-4">
          Ukur kemampuan<br />
          bahasa Indonesia<span className="text-[#C9A84C]">.</span><br />
          Secara <span className="text-[#C9A84C]">sesungguhnya</span>.
        </h1>

        <p className="text-white/60 text-base md:text-lg max-w-xl mx-auto mb-8 leading-relaxed">
          UKBI-Next adalah sistem penilaian kemahiran Bahasa Indonesia generasi baru — berbasis CEFR, didukung AI, dan mengukur apa yang bisa kamu lakukan, bukan sekadar apa yang kamu hafal.
        </p>
        ...
        <div className="flex items-center justify-center gap-2 max-w-md mx-auto mb-8">
          {LEVELS.map((level, i) => (
            <div key={level.code} className="flex-1 flex flex-col items-center gap-1.5">
              ...
              />
              <span className={`text-[11px] font-semibold tracking-wide transition-colors duration-300 ${i <= activeStep ? 'text-white/80' : 'text-white/30'}`}>
                {level.code}
              </span>
            </div>
          ))}
